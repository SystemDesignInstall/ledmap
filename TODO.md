...
- [x] План/docs-gate 7C принят: [LEDMAP-PROJECT-VALIDATION-001](docs/specs/LEDMAP-PROJECT-VALIDATION-001.md), версия 1.0 Accepted; ADR-022. Single-profile runtime input, shape → 7A → 7B, immutable diagnostics/checks, first-failure semantic stages с явным blocked; без warnings и всеобщего semantic aggregation.
- [ ] 7C production `validation`: реализация не разрешена; ожидает отдельного разрешения после проверки SHA docs-only acceptance-коммита. Сохранить baseline 625 тестов и принятые контракты движков.
