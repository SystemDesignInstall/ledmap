...
- **Статус:** Accepted — CLOSED. Docs-gate baseline `6fdb9aa39d303e6d9f62fd6289be6d68e4ad43a9`; принятая production implementation `09abe2386356073a89d75d7cf91a023abd94f9f4`; required production corrective `1fe2516ae1274fddd8271366946eb5159122b151` (collision `AddressingProfile.identity.id` с `identity.id` bundle → `PROFILE_DUPLICATE_ID` на `["addressingProfile","identity","id"]`, фаза duplicate ids); implementation record / docs alignment `b27fe0462f0f7a1c35cb227b7c1b378541dd7c1b` (`§6.2` приведён к `maxPorts` required). Ветка `6fdb9aa → 09abe23 → 1fe2516 → b27fe04` fast-forward в `master`, `origin/master` = `b27fe04`. Baseline Phase 7 CLOSED — `6bf98b437c3f0c1371e1636a578dce917168c76e`; regression 1087 тестов сохранены, добавлено 151: 1202/1202 core-only, 1238/1238 full, локальные результаты без CI-подтверждения. Два спорных решения приняты окончательно: `ProcessorProfile.maxPorts` обязателен, `LEDMAP-GENERIC-REF001` — нормативное имя профиля, а `LEDMAP_GENERIC_REF001` — корректный TypeScript export identifier.
...
- **Идентификация и ссылочная целостность 7E:** identity-bearing constituent profiles — `ProcessorProfile`, `PortProfile`, `ReceiverProfile`, `ModuleProfile`, `AddressingProfile`; каждый несёт `identity: ProfileIdentity`. `PixelTransportProfile` — **embedded bundle configuration** и в 7E собственного `ProfileIdentity` не имеет: его версия определяется `HardwareProfileBundle.identity.version`, а связь с геометрией задаётся `moduleProfileId`. Bundle содержит ровно один `AddressingProfile`, поэтому `ProcessorProfile.addressingProfileId` MUST совпадать с `bundle.addressingProfile.identity.id`. Lookup по `id` — единственный разрешающий механизм; отсутствующая ссылка — ошибка, а не fallback.
...
- **Production closure:** реализация принята в `09abe23` с обязательным corrective `1fe2516`; отчёт о реализации и выравнивание docs — `b27fe04`. Scope production строго additive и не расширен: 18 файлов (6 source + export + 10 test/fixtures + отчёт), 6A–7D, app, `.ledmap` schema, `Receiver.pixelCapacity`, `resolveHardware()` и `validateProject()` не изменены, новых runtime-зависимостей нет. `test:smoke` перепрогнан на финальном кандидате `b27fe04` — PASS. Следующие gates staged sequence: Address Encoder, затем `LEDMAP-FINAL-REMAP-REVERSEINDEX-SPEC-001`; оба не авторизованы этим ADR.
