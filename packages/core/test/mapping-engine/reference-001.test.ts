...
import {
  resolveMapping, mapInputPixel, unmapHardwarePixel, globalRemapIndex, asProcessorId, asPortId,
  type MappedPixel,
} from '../../src/index.js'
import { deepFreeze } from '../hardware-engine/fixtures.js'
import { expectedReference, referenceMapping } from './fixtures.js'
...
describe.each([false, true])('REF-001 mapping offset=%s', offset => {
  const mapping = resolveMapping(deepFreeze(referenceMapping(offset)))

  it.each(anchors)('maps anchor ($x,$y) in both directions', anchor => {
    ...
    expect(mapInputPixel(mapping, { inputCanvas: mapping.inputCanvas.id, inputCoordinate })).toEqual(expected)
    expect(unmapHardwarePixel(mapping, key)).toEqual(expected)
    expect(globalRemapIndex(mapping.hardware, key)).toBe(anchor.global)
  })

  it('sweeps all 196608 input pixels against independent expectations and unique hardware keys', () => {
    const keys = new Set<string>()
    for (let y = 0; y < 384; y += 1) {
      for (let x = 0; x < 512; x += 1) {
        const expected = expectedReference(x, y, offset)
        const actual = mapInputPixel(mapping, { inputCanvas: mapping.inputCanvas.id, inputCoordinate: expected.inputCoordinate })
        ...
        assertMappedPixel(unmapHardwarePixel(mapping, key), expected)
      ...
      for (const cabinet of port.cabinets) {
        for (let module = 0; module < 16; module += 1) {
          for (let pixel = 0; pixel < 1024; pixel += 1) {
            ...
            const actual = unmapHardwarePixel(mapping, key)
